...
| 6 | Flight supply | `supplier.flight` | ✅ pipeline/pricing/booking | ✅ catalog | ✅ `/api/flights/search` | ✅ | seeded catalog | ✅ | ⚡ **LIVE via Parto cache (2026-09-12/13): Parto CRS API v3 or portal-scrape → FlightOfferCache, stale-while-revalidate; needs live creds/Iran IP** | same | **BETA (live cache)** | `src/services/flight-cache-service.ts`, `src/domains/supplier/adapters/PartoPortalProvider.ts` |
...
| 9 | FX | `fx.live-gated` | ✅ Decimal + snapshot; **LIVE feed chain (2026-09-13): tgju market → configured fxapi → static fallback, fail-closed, honest source labels** | ✅ persisted rates | — | ✅ | static fallback on failure | ✅ | ✅ live-capable (needs network) | same | **BETA (live-gated)** | `src/domains/currency/CurrencyService.ts` (LiveFxRateProvider) |
