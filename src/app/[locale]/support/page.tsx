...
export default function SupportPage() {
  ...
  const FAQS: Array<{ q: string; a: string }> = cfg.faqOverride ?? [
    ...
    {
      q: lt(locale, { fa: 'چرا رزرو خدمات و پرداخت با کارت شتاب یا کیف‌پول به صورت خودکار تایید می‌شود؟', en: 'How are bookings automatically confirmed with Shetab and wallet payments?', ar: 'كيف يتم تأكيد الحجوزات تلقائياً مع الدفع بشتاب والمحفظة؟', zh: '为什么通过 Shetab 和钱包可以自动确认订单？', ru: 'Как автоматически подтверждаются бронирования при оплате Shetab и кошельком?' }),
      a: lt(locale, { fa: 'پلتفرم فیروزو با بهره‌گیری از موتور قیمت‌گذاری ۱۲ مرحله‌ای و دفتر کل مالی دوطرفه، بلافاصله پس از تایید تراکنش درگاه امن، سفارش را ثبت و واچر رسمی دیجیتال همراه با کد پیگیری یکتا را صادر می‌کند.', en: 'Firuzo uses a 12-stage server pricing engine and double-entry ledger, issuing an official digital voucher and unique tracking code immediately upon payment clearance.', ar: 'تستخدم فيروزو محرك تسعير من 12 مرحلة مع دفتر أستاذ مؤتمت، وتصدر قسيمة رقمية رسمية فور تأكيد الدفع.', zh: 'Firuzo 采用12阶段定价引擎与复式记账流水，支付成功后即刻出具官方数字凭证与唯一追溯码。', ru: 'Firuzo использует 12-ступенчатый движок ценообразования и выдает официальный ваучер сразу после подтверждения оплаты.' }),
    },
    {
