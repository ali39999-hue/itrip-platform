...
export const CHECKIN = '2026-09-22';
export const CHECKOUT = '2026-09-26';
...
export function nightsOf(a: string, b: string): Date[] {
  ...
  for (let d = new Date(a + 'T00:00:00'); d < end; d = new Date(d.getTime() + 864e5)) out.push(new Date(d));
  return out;
}

export const NIGHTS = nightsOf(CHECKIN, CHECKOUT);
...
export function nightlyRates(room: RoomDef, plan: PlanId) {
...
export function quote(room: RoomDef, plan: PlanId, childrenInRoom = 0) {
  const nts = nightlyRates(room, plan);
  const sub = nts.reduce((s, n) => s + n.price, 0);
  const extraChild = Math.max(0, childrenInRoom - room.capC) * 350 * NIGHTS.length;
  const tax = Math.round((sub + extraChild) * TAX);
  return { nights: nts, sub, extraChild, tax, total: sub + extraChild + tax, avg: Math.round((sub + extraChild + tax) / NIGHTS.length) };
}
...
export function useHotelBooking() {
  ...
  const totals = useMemo(() => {
    ...
    Object.entries(sel).forEach(([k, q]) => {
      ...
      const qt = quote(r, pid, cInRoom);
      sub += qt.sub * q; tax += qt.tax * q; extra += qt.extraChild * q; total += qt.total * q;
    });
  ...
  const bestCombo = useMemo(() => {
    ...
    const opts: Opt[] = [];
    ROOMS.forEach((r) => r.plans.forEach((p) => opts.push({ r, p, cost: quote(r, p, Math.min(CHILDREN, r.capC)).total })));
    ...
    const walk = (start: number, pick: Opt[]) => {
      ...
      for (let i = start; i < opts.length; i++) { pick.push(opts[i]); walk(i, pick); pick.pop(); }
    };
    walk(0, []);
