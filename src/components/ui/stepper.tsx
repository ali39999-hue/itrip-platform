...
export interface StepperProps {
  ...
  /** Accessible name for the step list (pass lt(locale, …) from the caller). */
  ariaLabel?: string;
  className?: string;
}

/** مراحل. Completed steps get a check, the current one a ring; flows right-to-left. */
export function Stepper({ steps, current, orientation = "horizontal", ariaLabel = "مراحل", className }: StepperProps) {
  const horizontal = orientation === "horizontal";
  return (
    <ol className={cn(horizontal ? "flex items-start" : "flex flex-col gap-5", className)} aria-label={ariaLabel}>
      {steps.map((s, i) => {
