...
import type { FaqItemOverride } from '@/domains/content/SiteContentService';

export const DEFAULT_FAQ: FaqItemOverride[] = [
  {
    q: {
...
export function FaqSection({ override }: { override?: FaqItemOverride[] }) {
  const locale = useLocale();
  const [openIdx, setOpenIdx] = useState<number | null>(0);

  const faqs = (override ?? DEFAULT_FAQ).map((item) => ({
    q: lt(locale, item.q),
    a: lt(locale, item.a),
  }));
