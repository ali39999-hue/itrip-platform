...
test.describe('i18n Language Switching across pages', () => {
  for (const pagePath of PAGES_TO_TEST) {
    test(`Should be able to load and switch languages on ${pagePath}`, async ({ page }) => {
      ...
      const response = await page.goto(`/fa${pagePath === '/' ? '' : pagePath}`, { waitUntil: 'domcontentloaded' });
      ...
      const langSwitcherBtnAfter = page.getByRole('button', { name: /Language|زبان/i }).first();
